...
type
  ...
  Public
   function  acessaPessoaMov(Sender:TObject;nCodEmp,nCodUsu:integer;cNome:string):Boolean;
...
uses UFuncoes,unit1,uUsuario,UUsuarioMov;

{$R *.dfm}

procedure TDMEmp.DataModuleCreate(Sender: TObject);
begin
DMEmp.SQLConnection1.Connected:=true;
...
//DMEmp.HoraConexao(Now,'Conexão inicial','marcioraposo','marcioraposo','Rent*987','192.168.0.1');
end;
...
function TDMEmp.AcessaPessoaMov(Sender:TObject;nCodEmp,nCodUsu:integer;cNome:string):Boolean;
var
obj:TFUsuarioMov;
begin
TabelaFindKey('pessoausuario',DMEmp.TBusuario,['ID'],[IntToStr(nCodUsu)]);
obj:=TFUsuarioMov.Create(Self);
try
obj.Caption:='Usuário cadastrado';
Obj.EdNome.Text  :=cNome;
Obj.EdAcesso.Text:=DMEmp.TBusuarioAcesso.Value;
Obj.EdSenha.Text:=DMEmp.TBusuarioSenha.Value;
Obj.ShowModal;
if obj.Tag=1 then
   begin
   DMEmp.TBusuario.Edit;
   DMEmp.TBusuarioAcesso.Value:=Obj.EdAcesso.Text;
   DMEmp.TBusuarioSenha.Value :=Obj.EdSenha.Text;
   DMEmp.TBusuario.Post;
   DMEmp.UpCliDataSet(DMEmp.TBusuario);
   end;
finally
end;
end;
